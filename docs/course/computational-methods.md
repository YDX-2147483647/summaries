...
> :material-clock-edit-outline: 2024年11月27日。

- 边界条件规定函数值时，没必要用虚拟网格；固定导数值时，才绕不开虚拟网格。
- 虚拟网格是在边界列两个方程，一个是微分方程，一个是边界条件。这俩方程除了用到边界点、相邻的内点，还会用到边界外的点。可以用两方程消掉边界外的点，剩下一个方程。这样总方程数和不虚拟的方法一样。
...
> :material-clock-edit-outline: 2024年11月20日。
...
- 局部截断误差的定义是差分减微分，不是反过来，而且没有与网格长度有关的倍数。
- 有限元法选的基有两段非零，每个单元刚度阵只用一段，加起来总刚度阵才用两段。
