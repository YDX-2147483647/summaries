...
- Sinclair散射矩阵：Jones矢量从输入到输出的线性变换——四个复数自由度（$\C^2 \to \C^2$），可能冗余自由度（互易过程的对称，无耗过程的共轭对称，匹配传输时无对角元，缺少定标时绝对相位不定）
- Müller矩阵：Stokes参数从输入到输出的线性变换——十六个实数自由度（$\R^4 \to \R^4$），可能冗余自由度
